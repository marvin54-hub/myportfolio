    ...
    <>
      ...
      <div className="layout">
        ...
        <main className="main">
          ...
          <Projects
            projects={(Array.isArray(projects) ? projects : PROJECTS).map(p => {
              if (!p) return p
              const hasImages = (Array.isArray(p.screenshots) && p.screenshots.filter(Boolean).length > 0) || Boolean(p.screenshot)
              if (!hasImages) {
                const match = PROJECTS.find(dp => dp.id === p.id || dp.name === p.name)
                if (match && match.screenshots) return { ...p, screenshots: match.screenshots }
              }
              return p
            })}
